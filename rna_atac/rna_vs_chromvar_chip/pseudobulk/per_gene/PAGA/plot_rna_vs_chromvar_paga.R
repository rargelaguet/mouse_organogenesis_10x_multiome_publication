...
io$outdir <- file.path(io$basedir,sprintf("results/rna_atac/rna_vs_chromvar_chip/pseudobulk/per_gene/%s/pdf/paga",opts$motif_annotation))
...
for (i in genes.to.plot) {
  ...
  if (file.exists(outfile)) {
    ...
    pdf(outfile, width = 6, height = 3.25)
    print(p.all)
    dev.off()
  }
}
...
#   
#   if (opts$scale) {
#     outfile <- file.path(io$outdir,sprintf("%s_paga_rna_vs_chromvar_chip_scaled.png",i))
#   } else {
#     outfile <- file.path(io$outdir,sprintf("%s_paga_rna_vs_chromvar_chip.png",i))
#   }
#   
#   if (file.exists(outfile)) {
#     print(sprintf("file or %s already exists...",i))
