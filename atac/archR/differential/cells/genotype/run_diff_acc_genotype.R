here::i_am("atac/archR/differential/cells/genotype/run_diff_acc_genotype.R")
...
# # args$group_variable <- "celltype_genotype"
# args$min_cells <- 30
# args$outdir <- file.path(io$basedir,sprintf("results/atac/archR/differential/cells/celltype_genotype/%s",args$matrix))
# args$test_mode <- TRUE
## END TEST ##
...
cells_metadata.dt <- fread(args$metadata) %>%
  .[sample%in%opts$samples & pass_atacQC==TRUE & !is.na(genotype) & !is.na(celltype)] %>%
...
# Only consider cell types with sufficient number of cellls
stats.dt <- cells_metadata.dt[,.N,by=c("celltype","genotype")] %>% dcast(celltype~genotype, value.var="N", fill=0)
...
for (j in celltypes.to.use) {
  ...
  if (!file.exists(outfile)) {
    ...
    cmd <- sprintf("%s 'Rscript %s --archr_directory %s --metadata %s --samples %s --celltypes %s --groupA WT --groupB T_KO --matrix %s --group_variable genotype --statistical_test %s --outfile %s'",
      lsf, io$script, args$archr_directory, args$metadata, paste(opts$samples,collapse=" "), j, args$matrix, opts$statistical.test, outfile)
