...
io$outdir <- file.path(io$basedir,"results/atac/archR/metacells/pdf"); dir.create(io$outdir, showWarnings = F)

dir.create(file.path(io$outdir,"individual_peaks"), showWarnings = F)
...
for (i in features.to.plot) {
  ...
  pdf(file.path(io$outdir,sprintf("individual_peaks/%s_cell_metacell_pseudobulk_acc.pdf",gsub("[:-]","_",i))), width=10, height=10)
  print(p)
  dev.off()
}
