...
## START TEST ##
args$query_samples <- c("E8.5_CRISPR_T_KO", "E8.5_CRISPR_T_WT")
...
args$atlas_sce <- file.path(io$atlas.basedir,"results/trajectories/nmp_somitic_spinal/nmp_SingleCellExperiment.rds")
args$atlas_metadata <- file.path(io$atlas.basedir,"results/trajectories/nmp_somitic_spinal/nmp_sample_metadata.txt.gz")
args$npcs <- 10
args$n_neighbours <- 15
args$trajectory_name <- "NMP"
args$outfile <- paste0(io$basedir,"/results/rna/mapping/trajectories/nmp_somitic_spinal/sample_metadata_after_mapping.txt.gz")
## END TEST ##
...
opts$celltype_trajectory_dic <- list(
  ...
  "NMP" = c("Somitic_mesoderm","Caudal_Mesoderm","NMP","Spinal_cord")
)
...
# Load SingleCellExperiment
sce_atlas <- load_SingleCellExperiment(args$atlas_sce, normalise = FALSE, cells = meta_atlas$cell, remove_non_expressed_genes = TRUE)
...
mapping.dt <- mapping$mapping %>% 
  ...
  merge(meta_query[,c("cell","sample","genotype")],by="cell") %>%
  as.data.table
