here::i_am("rna/differential/metacells/parse_differential_results.R")
...
# args$min_cells <- 5
# args$outdir <- file.path(io$basedir,"results/rna/differential/metacells/celltype")
## END TEST ##
  ...
  for (j in i:length(opts$celltypes)) {
    ...
    if (i!=j) {
      ...
      if (file.exists(file)) {
        ...
        # Empty file (not enough cells to do DE)
        if (nrow(tmp)>1) {
...
fwrite(stats.dt[-1], file.path(args$outdir,"diff_expr_stats.txt.gz"), sep="\t", quote=F, na="NA")
